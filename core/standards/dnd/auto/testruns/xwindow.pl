...
my @fail;
my @skip;
DnD::GrabWindow('Desktop');
DnD::GrabWindow('TV');
for my $testrun (1..2)
	{open(TCs,"xwindow$testrun.txt") or die("Can not find list of testcases, make sure that xwindow$testrun.txt is located in the same directory as this script.\n");
	while (<TCs>)
		{$_ =~ s/[\n\r]+//;
		DnD::LoadPage($_);
		...
		if($dropX != 0 and $dropY != 0)
			...
			if($dragX != 0 and $dragY != 0)
				...
				DnD::DragBetweenWindows($dragX,$dragY,$dropX,$dropY);
				if(DnD::Green)
					{print "PASS\n";
					$pass = $pass + 1;}
				else
