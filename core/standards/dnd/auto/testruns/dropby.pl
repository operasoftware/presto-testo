...
DnD::GrabWindow('Gogi');
for my $testrun (1..2)
	{open(TCs,"dropby$testrun.txt") or die("Can not find list of testcases, make sure that dropby$testrun.txt is located in the same directory as this script");
	while (<TCs>)
		{$_ =~ s/[\n\r]+//;
		DnD::LoadPage($_);
		my ($dragA,$dragB,$dragC,$dragD,$dropA,$dropB,$dropC,$dropD) = DnD::ScanPage;
		my ($dragX,$dragY,$dropX,$dropY) = (int(($dragA+$dragC)/2),int(($dragB+$dragD)/2),$dropA+8,$dropB+8);
		if($dragX != 0 and $dragY != 0 and $dropA != 0 and $dropB != 0)
			{if($testrun == 2)
