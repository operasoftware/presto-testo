...
my @skip;
DnD::GrabWindow('Gogi');
open(TCs,'interrupt.txt') or die('Can not find list of testcases, make sure that interrupt.txt is located in the same directory as this script');
while (<TCs>)
	{$_ =~ s/[\n\r]+//;
	DnD::LoadPage($_);
	my ($dragA,$dragB,$dropA,$dropB,$dragC,$dragD,$dropC,$dropD) = DnD::ScanPage;
	...
	if($dragX != 0 and $dragY != 0 and $dropX != 0 and $dropY != 0)
		{DnD::DisturbDuringDrag($dragX,$dragY,$dropX,$dropY);
		if(DnD::Green)
			{print "PASS\n";
			$pass = $pass + 1;}
		else
