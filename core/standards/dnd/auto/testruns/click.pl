...
DnD::GrabWindow('Gogi');
for my $testrun (0..4)
	{open(TCs,"click$testrun.txt") or die("Can not find list of testcases, make sure that click$testrun.txt is located in the same directory as this script");
	while (<TCs>)
		{$_ =~ s/[\n\r]+//;
		DnD::LoadPage($_);
		...
		if($dragX != 0 and $dragY != 0 and $dropX != 0 and $dropY != 0)
			{if($testrun == 0)
				...
				{DnD::Click($dragA+5,$dragB+5,$testrun);}
			if($testrun == 1)
				{DnD::SelectAll;}
