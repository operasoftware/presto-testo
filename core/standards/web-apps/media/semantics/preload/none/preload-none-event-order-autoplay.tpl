...
<script>
var tests = init_tests("HTMLAudioElement.preload:$start_state event order when autoplay content attribute is set - $media_type", "HTMLVideoElement.preload:$start_state event order when autoplay content attribute is set - $media_type", {timeout:$timeout});
