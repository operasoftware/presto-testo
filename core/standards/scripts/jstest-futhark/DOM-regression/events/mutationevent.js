...
{
  var cvs = "$Id: mutationevent.js 25285 2008-04-04 12:44:41Z hallvord $";
  ...
  try
  {
    ...
    if (document.implementation.hasFeature("MutationEvents", "2.0"))
    {
      ...
      function handleDOMNodeRemovedFromDocument(ev)
      {
        ...
        test("ev.type #"+noderemovedfromdocument, ev.type, "DOMNodeRemovedFromDocument");
        ttrue("ev.target #"+noderemovedfromdocument, ev.target == div2 || ev.target == div3);
        test("ev.relatedNode #"+noderemovedfromdocument, ev.relatedNode, null);
      }
      ...
      function handleDOMNodeInsertedIntoDocument(ev)
      {
        ...
        test("ev.type #"+nodeinsertedintodocument, ev.type, "DOMNodeInsertedIntoDocument");
        ttrue("ev.target #"+nodeinsertedintodocument, ev.target == div2 || ev.target == div3);
        test("ev.relatedNode #"+nodeinsertedintodocument, ev.relatedNode, null);
      }
      ...
      {
        test("ev.type #"+attrmodified, ev.type, "DOMAttrModified");
        test("ev.target #"+attrmodified, ev.target, div1);
        test("ev.attrName #"+attrmodified, ev.attrName, "class");

        switch (++attrmodified)
        {
        case 1:
          test("ev.attrChange case 1", ev.attrChange, 2);
          test("ev.prevValue case 1", ev.prevValue, "");
          test("ev.newValue case 2", ev.newValue, "class1");
          break;
          
        case 2:
          test("ev.attrChange case 2", ev.attrChange, 1);
          test("ev.prevValue case 2", ev.prevValue, "class1");
          test("ev.newValue case 2", ev.newValue, "class2");
          break;
        }
      }
