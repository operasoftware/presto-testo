...
var cvs = "$Id: importNode.js 86201 2011-01-14 13:27:20Z jgraham $";

testmodule("Import Node", cvs);

try {

  var w = document.getElementById('importnode2').contentWindow ; // window.open( 'importNode2.html', "import-node-win" );
  if (!w)
  {
    ...
    function runTests()
    {
      ...
      else
      {
        ...
        expect_DOM_exception( "Element #19", DOMException.WRONG_DOCUMENT_ERR,
                              function(){ localSpan.appendChild( remoteElement ); localSpan.removeChild(localSpan.lastChild); }, 'Expected to fail for a while - see #242345' ); /* added a removeChild because if we do allow this it otherwise messes up tests further on */
        ...
        test( "Attribute #13", localSpan.attributes.item(0).name.toUpperCase(), "ID" );
        ...
        localSpan.setAttributeNode( impAttr );
        test( "Attribute #21", localSpan.attributes.item(0).name.toUpperCase(), "ID" );
        ...
        expect_DOM_exception( "Document Fragment #18", DOMException.WRONG_DOCUMENT_ERR,
                              function(){ localSpan.appendChild( remoteDocFrag ); localSpan.removeChild( localSpan.lastChild ); }, 'Expected to fail for a while - see #242345' ); /* added removeChild because the extra element confuses latter tests */
        ...
        expect_DOM_exception( "Text #16", DOMException.WRONG_DOCUMENT_ERR,
                              function(){ localSpan.appendChild( remoteText ); localSpan.removeChild(localSpan.lastChild); }, 'Expected to fail for a while - see #242345' );
        ...
        expect_DOM_exception( "Input #1", DOMException.NO_DATA_ALLOWED_ERR, function(){ document.importNode( true, remoteText );} );

        expect_DOM_exception( "Input #2", DOMException.NO_DATA_ALLOWED_ERR, function(){ document.importNode( "test", true ); });
        expect_DOM_exception( "Input #3", DOMException.NO_DATA_ALLOWED_ERR, function(){ document.importNode( 1, false ); });
        expect_DOM_exception( "Input #4", DOMException.NO_DATA_ALLOWED_ERR, function(){ document.importNode( true ); });
        expect_DOM_exception( "Input #5", DOMException.NO_DATA_ALLOWED_ERR, function(){ document.importNode(); });
      }
