...
;                          rewrote to eliminate property lists,
;                          and added a scaling parameter suggested by Bob Boyer)
; Language:     Scheme
...
; Note:  The version of this benchmark that appears in Gabriel's book contained
; several bugs that are corrected here.  These bugs are discussed by Henry Baker,
; "The Boyer Benchmark Meets Linear Logic", ACM SIGPLAN Lisp Pointers 6(4),
; October-December 1993, pages 3-10.  The fixed bugs are:
;
...
function nboyer_benchmark ()
{
	var n = (arguments.length == 0 ? 0 : arguments[1]);
    setup_boyer();
    test_boyer(n);
}

var setup_boyer true;  // assigned below
var test_boyer true;   // assigned below
...
{  
  ...
  {
	add_lemma_lst(
	// This is almost right, if we define codes for all the symbols and 
	// we put $ in front of some more idents here.
...
[[$equal, [$compile, $form],
 ...
 [$equal, [$equal, $x [$difference, $x $y]],
	  [$and, [$numberp, $x],
		 [$or, [$equal, $x [$zero]],
		       [$zerop, $y]]]],
 [$equal, [$meaning, [$plus_tree, [$append, $x $y]],
		     a],
	  [$plus, [$meaning, [$plus_tree, $x],
			     a],
		  [$meaning, [$plus_tree, $y],
			     a]]],
 [$equal, [$meaning, [$plus_tree, [$plus_fringe, $x]],
		     a],
	  [$fix, [$meaning, $x a]]],
 ...
 [$equal, [$exp, i [$times, j k]],
	  [$exp, [$exp, i j],
		 k]],
 [$equal, [$reverse_loop, $x $y],
	  [$append, [$reverse, $x],
		    $y]],
 [$equal, [$reverse_loop, $x [$nil]],
	  [$reverse, $x]],
 ...
 [$equal, [$plus, [$remainder, $x $y],
	  ...
	  [$fix, $x]],
 [$equal, [$power_eval, [$big_plus1, l i base],
			base],
	  [$plus, [$power_eval, l base],
		  i]],
 [$equal, [$power_eval, [$big_plus, $x $y i base],
			base],
 ...
 [$equal, [$lessp, [$remainder, $x $y],
	  ...
	  [$and, [$not, [$zerop, $y]],
		 [$not, [$zerop, $x]],
		 [$not, [$lessp, $x $y]]]],
 [$equal, [$power_eval, [$power_rep, i base],
			base],
	  [$fix, i]],
 [$equal, [$power_eval, [$big_plus, [$power_rep, i base],
				    [$power_rep, j base],
				    [$zero],
				    base],
			base],
 ...
 [$equal, [$samefringe, $x $y],
	  [$equal, [$flatten, $x],
		   [$flatten, $y]]],
 [$equal, [$equal, [$greatest_factor, $x $y],
		   [$zero]],
	  [$and, [$or, [$zerop, $y],
		       [$equal, $y 1]],
		 [$equal, $x [$zero]]]],
 ...
 [$equal, [$equal, z [$times, w z]],
	  [$and, [$numberp, z],
		 [$or, [$equal, z [$zero]],
		       [$equal, w 1]]]],
 [$equal, [$greatereqp, $x $y],		;########## doesn't matter
	  [$not, [$lessp, $x $y]]],
 ...
 [$equal, [$equal, [$difference, $x $y],
	  ...
	  [$if, [$lessp, $x $y],
		...
		[$if, [$lessp, z $y],
		      ...
		      [$equal, [$fix, $x],
			       [$fix, z]]]]],
 [$equal, [$meaning, [$plus_tree, [$delete, $x $y]],
		     a],
	  [$if, [$member, $x $y],
		[$difference, [$meaning, [$plus_tree, $y],
					 a],
			      [$meaning, $x a]],
		[$meaning, [$plus_tree, $y],
			   a]]],
 ...
 [$equal, [$equal, [$lessp, $x $y],
		   z],
	  [$if, [$lessp, $x $y],
		[$equal, [$t], z],	//########## doesn't matter
		[$equal, [$f], z]]],	//########## doesn't matter
 ...
 [$equal, [$get, j [$set, i val mem]],
  ...
  ; For debugging only, so MAP can be used without changing
  ...
  (define (test n)           ;########## scaling parameter added
    (let ((term              ;##########
           (apply-subst
            (translate-alist ;##########
  ...
  (define (apply-subst alist term)
    (cond ((not (pair? term))
           (let ((temp-temp (assq term alist))) ;########## was global
             (if temp-temp
                 (cdr temp-temp)
                 term)))
          (else (cons (car term)
  ...
  (define (one-way-unify1 term1 term2)
    (cond ((not (pair? term2))
           (let ((temp-temp (assq term2 unify-subst))) ;########## was global
             (cond (temp-temp
                    (term-equal? ;########## was equal?
                     term1 (cdr temp-temp)))
  ...
  (define (falsep x lst)
    (or (term-equal?   ;########## was equal?
         x false-term) ;##########
        (term-member?  ;########## was member
         x lst)))
  ...
  ;########## The remaining procedures are new code.
