...
var cvs = "$Id: regExpTest.js 79397 2010-09-29 03:52:37Z hallvord $";

function main()
{
   try
   {
      testmodule( "RegExp testsuite", cvs );
      expect(407);//number of tests - update this when tests are added!
      testcase( 'Regexp creation' );                   testCreation();
      testcase( 'Miscellaneous regular expressions' ); testRegExps();
      testcase( 'Regexp regression testing' );         testRegression();
...
function testCreation()
{
    ...
    try
	{
		...
		re = /abra/;
		  ...
		  test( "Constructor #14", new RegExp(new String("abra"), new String("gi")).toString(), "/abra/gi" );

		// Compile
		re=/abra/;
		  test( "Compile #1", re.compile(), undefined );
		  test( "Compile #2", re.compile("zappa"), undefined );
		  test_deep( "Compile #3", re.exec("frankzappa"), ["zappa"] );
		  test( "Compile #4", re.compile(new String("moon")), undefined );
		  test_deep( "Compile #5", re.exec("moonunit"), ["moon"] );
		  test( "Compile #6", re.compile(new Boolean(true)), undefined );
		...
		for ( var p in re )
			e[e.length] = p;
		test( "enumeration across regex #1", e.length, 0 );  // ECMA-3 section 15: attributes default to DontEnum
...
//
// 'g' is added to the flags if start != 0 (unless 'g' is in flags :-)

function re_test( pattern, input, start, loc, expected, flags )
{
    re_test.invocations = re_test.invocations||0; re_test.invocations++;

    testcase( 're_test #'+re_test.invocations+', '+pattern+' on '+input );
    ...
    try
	{
		...
		if (result !== null)
		{
			...
			test( "RegExp exec #4 on"  + re + ' and ' + input, result.input, ''+input ); /* result.input is a string so ''+input will make sure string is compared to string. Fixes test bug 271483. hallvord@opera.com 2007-07-03 */
		}
...
function testRegExps()
{
   ...
   try
   {
       ...
       if (typeof opera == 'undefined' || !opera.jsshell) {
           ...
           re_test("ø","ø",0,0,["ø"],'i');	// o-slash (U+00F8, U+00F8, U+00F8)
           re_test("ø","Ø",0,0,["Ø"],'i');	// o-slash (U+00F8, U+00D8, U+00D8)
           re_test("ø","Ø",0,0,null);	    // o-slash (U+00F8, U+00D8)
           re_test("ñ","Ñ",0,0,["Ñ"],'i');	// tilde-n (U+00F1, U+00D1, U+00D1)
       }
	   ...
	   re_test( /.......\8/, "aabbccaaa\x087654321", 0, -1, null );
	   re_test( /\9/, "\t12", 0, -1, null );
	   re_test( /\011/, "\t12", 0, 0, ["\t"] );
	   re_test( /\0111/, "\t12", 0, -1, null );
	   re_test( /\44/, "123,456", 0, -1, null );
	   re_test( /\444/, "123,456", 0, -1, null );
...
if (! (isBytecoded() || decompilerRemoved() ))
		{
			...
			test( "first letter of function name #1", RegExp.$1, "M" );

			function MyClass() {}
			var re = /function (\w)/;
			var x = re.test( MyClass );
			test( "first letter of function name #2", x, true );
		}
		...
		*/
		...
		var x = /(([a-zA-Z]([a-zA-Z0-9+-.])*):((((\/\/(((((%[0-9A-Fa-f]{2})|[a-zA-Z0-9\-_.!~*'();:&=+$,])*@)?((((([a-zA-Z0-9]|([a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9]))\.)*([a-zA-Z]|([a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9]))\.?)|(\d+\.\d+\.\d+\.\d+))(:\d*)?))?|((%[0-9A-Fa-f]{2})|[a-zA-Z0-9\-_.!~*'()$,;:@&=+])+)(\/((([a-zA-Z0-9\-_.!~*'():@&=+$,]|(%[0-9A-Fa-f]{2}))*(;(([a-zA-Z0-9\-_.!~*'():@&=+$,]|(%[0-9A-Fa-f]{2}))*))*)(\/(([a-zA-Z0-9\-_.!~*'():@&=+$,]|(%[0-9A-Fa-f]{2}))*(;(([a-zA-Z0-9\-_.!~*'():@&=+$,]|(%[0-9A-Fa-f]{2}))*))*))*))?)|(\/((([a-zA-Z0-9\-_.!~*'():@&=+$,]|(%[0-9A-Fa-f]{2}))*(;(([a-zA-Z0-9\-_.!~*'():@&=+$,]|(%[0-9A-Fa-f]{2}))*))*)(\/(([a-zA-Z0-9\-_.!~*'():@&=+$,]|(%[0-9A-Fa-f]{2}))*(;(([a-zA-Z0-9\-_.!~*'():@&=+$,]|(%[0-9A-Fa-f]{2}))*))*))*)))(\?(([a-zA-Z0-9\-_.!~*'();\/?:@&=+$,]|(%[0-9A-Fa-f]{2}))*))?)|(((%[0-9A-Fa-f]{2})|[a-zA-Z0-9\-_.!~*'();?:@&=+$,])([a-zA-Z0-9\-_.!~*'();\/?:@&=+$,]|(%[0-9A-Fa-f]{2}))*)))$/;
        ...
        test( "Compatible fallback for bad $n sequence in replace string #1",
              "D_Month=200509".replace(/(D_Month=)[0-9][0-9][0-9][0-9][0-9][0-9]/, "$1200506"),
              "D_Month=200506" );

        test( "Compatible fallback for bad $n sequence in replace string #2",
