...
var cvs = "$Id: stringObjectTest.js 47322 2009-06-04 13:43:30Z jgraham $";

function main()
{
    testmodule( "Strings and String objects", cvs );
    expect(283);//number of tests - update this when tests are added!
...
function testStringCast()
{
    ...
    x = String;    test_regexp( "String( String )", String( x ), ( decompilerRemoved() ? /function \(\) { \/\* source code not available \*\/ }/ : /function String\(\) {\s*\[native code\]\s*}/) );
}

function testStringConstruction()
{
    var str = "teststring";
    test( 'typeof str #1', typeof str, 'string' );
    test( 'instance of String #1', str instanceof String, false );

    str = new String( "teststring" );
    test( 'typeof str #2', typeof str, 'object' );
    test_regexp( 'constructor', String( str.constructor ),  ( decompilerRemoved() ? /function \(\) { \/\* source code not available \*\/ }/ : /function String\(\) {\s*\[native code\]\s*}/));
    test( 'constructor.length', str.constructor.length, 1 );
    test( 'instance of String #2', str instanceof String, true );
...
function testAnchor()
{
    ...
    test( 'anchor( name_attribute ) #1', anchstr, '&lt;A NAME="mylink_name"&gt;' + str + '&lt;/A&gt;' );

    str = "";
    anchstr = convertTags( str.anchor( '' ) );
    test( 'anchor( name_attribute ) #2', anchstr, '&lt;A NAME=""&gt;' + str + '&lt;/A&gt;' );
}
...
function testConcat()
{
    ...
    test( "x.concat( x ) #1", x.concat( x ), "barbar" );

    x = { toString: function () { return 'bar'; },
	  concat: String.prototype.concat
    };

    test( "x.concat( x ) #2", x.concat( x ), "barbar" );
...
{
    var str = "hello";
    test( 'constructor #1', str.construtor, undefined );

    str = new String( 'hello' );
  test_regexp( 'constructor #2', String( str.constructor ),  ( decompilerRemoved() ? /function \(\) { \/\* source code not available \*\/ }/ : /function String\(\) {\s*\[native code\]\s*}/) );
    test( 'constructor.length', str.constructor.length, 1 );
}
...
function testFontColor()
{
    ...
    test( 'fontcolor( color ) #1', fontcolorstr, '&lt;FONT COLOR="' + color + '"&gt;' + str + '&lt;/FONT&gt;' );

    color = "#cccccc"; // browsersafe gray
    fontcolorstr = convertTags( str.fontcolor( color ) );
    test( 'fontcolor( color ) #2', fontcolorstr, '&lt;FONT COLOR="' + color + '"&gt;' + str + '&lt;/FONT&gt;' );
...
function testFontSize()
{
    ...
    test( 'fontsize( size ) #1', fontsizestr, '&lt;FONT SIZE="' + size + '"&gt;' + str + '&lt;/FONT&gt;' );

    size = 10; // size as number
    fontsizestr = convertTags( str.fontsize( size ) );
    test( 'fontsize( size ) #2', fontsizestr, '&lt;FONT SIZE="' + size + '"&gt;' + str + '&lt;/FONT&gt;' );
...
function testIndexOf()
{
    ...
    test( 'indexOf #0', teststring.indexOf( 'doesntexist' ), -1 );
    test( 'indexOf #1', teststring.indexOf( 'My' ),  0 );
    test( 'indexOf #2', teststring.indexOf( '\!' ),  teststring.length - 1);
    test( 'indexOf #3', teststring.indexOf( 'that' ),  14 );
    // position 51 because of the escape character '\', which is not counted
    test( 'indexOf #4', teststring.indexOf( 'that', 15 ),  51 );
    ...
    test( 'indexOf #5', x.indexOf( 'ar' ), 1 );
}

function testLastIndexOf()
{
    var str = 'My teststring';
    test( 'lastIndexOf #1', str.lastIndexOf( 's' ), str.length - 6 );
    test( 'lastIndexOf #2', str.lastIndexOf( 'nonexisting' ), -1 );
    test( 'lastIndexOf #3', str.lastIndexOf( 's', str.length - 7 ), 5 );
    test( 'lastIndexOf #4', str.lastIndexOf( 'nonexisting', str.length - 7 ), -1 );
    test( 'lastIndexOf #5', str.lastIndexOf( 's', -7 ), -1 );
...
function testLink()
{
    ...
    test( 'link( name_attribute ) #1', linkstr, '&lt;A HREF\=\"http:\/\/mylink_href\"&gt;' + str + '&lt;/A&gt;' );

    str = "";
    linkstr = convertTags( str.link( '' ) );
    test( 'link( name_attribute ) #2', linkstr, '&lt;A HREF=""&gt;' + str + '&lt;/A&gt;' );
}
...
function testSlice()
{
    ...
    test( 'str.slice( start, end ) #1', str.slice( 0, str.length ), str );
    test( 'str.slice( start, end ) #2', str.slice( str.indexOf( 'to' ), str.indexOf( 'bone' ) - 1 ), 'to the' );
    ...
    test( 'str.slice( start, end ) #3', x.slice( 0, 3 ), 'Cho' );
    test( 'str.slice( start, end ) #4', x.slice( 2, 3 ), 'o' );
...
function testSplit()
{
    ...
    test_deep( "split #1a", "A<B>bold</B>and<CODE>coded</CODE>".split(/<(\/)?([^<>]+)>/),
	       [ "A", undefined, "B", "bold", "/", "B", "and", undefined, "CODE", "coded", "/", "CODE", ""] );
    test_deep( "split #1b", "ab".split(/a*?/), ["a","b"] );
    test_deep( "split #1c", "ab".split(/a*/), ["","b"] );
...
function testToLowerCase()
{
    var str = "AnALMOSTUPPeRCaseWORd";
    var num = 23;
    test( "str.toLowerCase() #1", "ABRACADABRA".toLowerCase(), "abracadabra" );
    test( "typeof str.toLowerCase()", typeof str.toLowerCase(), 'string' );
    test( "str.toLowerCase() #2", str.toLowerCase(), "analmostuppercaseword" );
