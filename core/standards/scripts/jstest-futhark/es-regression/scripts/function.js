...
var cvs = "$Id: function.js 57622 2009-11-17 16:02:51Z jgraham $";

testmodule( "Functions and Function objects", cvs );
expect(98);//number of tests - update this when tests are added!
var x;
try {
...
function usesarg1(x,y)  // receives three arguments
{
	...
	test( "usesarg1 #2", arguments.length, 3 );
}

test( 'function.arguments is undefined before function is called', usesarg1.arguments, undefined, 'Opera implementation detail, AFAIK undefined in spec' );
usesarg1( 'USESARG1_1', 'USESARG2_1', 'USESARG3_1' );
test( 'function.arguments is null after function is called', usesarg1.arguments, null, 'Opera implementation detail, AFAIK undefined in spec' );

function argLength(){return arguments.length};
function argLengthWithFuncName(){return argLengthWithFuncName.arguments.length};
test( 'arguments length, no args', argLength(), 0 );
test( 'arguments length, one arg', argLength('a'), 1 );
test( 'arguments length, null arg', argLength(null), 1 );
test( 'arguments length, array arg', argLength(['a', 'b']), 1 );
test( 'arguments length, apply with array args', argLength.apply(this,['a', 'b']), 2 );
test( 'arguments length, apply with array args #2', argLength.apply(this, new Array(100000) ), 100000 );
test( 'arguments length, call with mixed args', argLength.call(this,['a', 'b'], false, true), 3 );
test( 'arguments length (function name.arguments), no args', argLengthWithFuncName(), 0 );
test( 'arguments length (function name.arguments), one arg', argLengthWithFuncName('a'), 1 );

function forLoopOverArguments(){ for(var i=0,s='';i<arguments.length;i++){ s+=arguments[i]; }return s; }
function forInLoopOverArguments(){ var s=''; for( var p in arguments ){ return true; }return false; } /* should always return false, props should be don't enum. ECMA 262 10.1.8: For each non-negative integer, arg, less than the value of the length property, a property is created with name ToString(arg) and property attributes { DontEnum }. */
test( 'normal for loop over arguments (no arguments)', forLoopOverArguments(), '' );
test( 'normal for loop over arguments ', forLoopOverArguments('a', 'b', 'c'), 'abc' );
test( 'bigger for loop over arguments ', forLoopOverArguments.apply(this, new Array(1000)), new Array(1000).join('undefined')+'undefined' );
test( 'normal for in loop over arguments (no arguments)', forInLoopOverArguments(), false );
test( 'normal for in loop over arguments ', forInLoopOverArguments('a', 'b', 'c'), false );
test( 'bigger for in loop over arguments ', forInLoopOverArguments.apply(this, new Array(1000)), false );
...
{
    var av = eval(a);
    test( "function.arguments inside recursion: #"+counter+' - ' + a, av, bv );
    counter++;
}
...
function fn_with_formal_called_arguments2( i, arguments ) { return fn_with_formal_called_arguments2.arguments[0]; }
test( "Bug 140380 test #2", fn_with_formal_called_arguments2( 1, 2 ), 1 );
...
function argarray_reverse(a,b,c,d)
{
  var res = ""
  try {
	//this happens in futhark versions where arguments instanceof Array
	var res = arguments.reverse().toString();
  } catch(e) {
	//this happens in spec-compliant engines where !(arguments isntanceof Array)
	//They will fail this test but...
  }
  test("reversing the arguments array #1", res, "1,2,3,4");
