...
var cvs = "$Id: object.js 43149 2009-03-13 17:38:07Z hallvord $";

testmodule( "The Object object", cvs );
expect(78);//number of tests - update this when tests are added!
try {
	...
	test( "#2 numeric", x.hasOwnProperty(18), false );
...
testcase( "Literal object syntax" );\
 ...
 test( "Object syntax #3", x[3], "three", 73932 );\
 var y = { "001": "yes", 1: "no" }; \
 test( "Object syntax #4", y["001"], "yes", "#127773" ); \
 test( "Object syntax #5", y[1], "no", "#127773" ); \
';
    if (!isExplorer())
	eval(prog);
    else
    ...
    ttrue( "cannot change read-only #2", p );

    if (! (isBytecoded() || decompilerRemoved() ))
    {
	testcase( "Decompiling object literal" );
	...
	test_spaceagnostic( "Decompiling object literal #1",
			    myobj.toString(),
...
function doTestWhenIframeIsReady(){ // name says it all, no? OK, look at 299533
    ...
    testcase( "Properties of prototypes on implicit conversion (no bug#)" );
        ...
        test( "Correct prototype #2", resultFromIframe, "false,true,true,false" );
