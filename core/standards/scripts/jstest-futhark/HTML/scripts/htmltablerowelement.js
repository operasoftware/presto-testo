...
var cvs = "$Id: htmltablerowelement.js 108182 2012-01-13 12:09:02Z sigbjornf $";
...
try {
  ...
  test("align",row.align,'');// default value no longer expected, see bug 291905
  ...
  test("vAlign",row.vAlign,"");// default value no longer expected, see bug 291905
  ...
  expect_readonly_exception( "rowIndex", function(){"use strict"; row.rowIndex = 7;} )
  test("rowIndex",row.rowIndex,rowIndex);
  var tri = row.sectionRowIndex;
  expect_readonly_exception( "sectionRowIndex", function(){"use strict"; row.sectionRowIndex = 9;} )
  test("sectionRowIndex",row.sectionRowIndex,tri);
  var cells = row.cells;
  expect_readonly_exception( "cells", function(){"use strict"; row.cells = document.images;} )
  ...
  test("TD colspan #2", tdattr, null);// default value no longer expected, see bug 291905
  if(tdattr){
  ...
  test("TD rowspan #2", tdattr, null);// default value no longer expected, see bug 291905
  if(tdattr){
  ...
  test("TH colspan #2", thattr, null);// default value no longer expected, see bug 291905
  if(thattr){
  ...
  test("TD rowspan #2", thattr, null);// default value no longer expected, see bug 291905
  if(thattr){
