...
{
   var cvs = "$Id: js_window.js 41134 2009-02-13 14:10:19Z hallvord $";
...
function testWindowProperties( win )
{
   ...
   */
   ...
   test( "win.frames toString", String (win.frames), "[object Window]" ); // used to be object WindowCollection, changed due to CORE-6730
   ...
   try
   {
      ...
      win.document = null; // should be silently ignored
      test( "setting win.document to null", win.document, org_document );
...
function testWindowMethods( win )
{
   ...
   win.moveBy( 10, 5 );
   test( "moveBy #3", win.screenX, ( oldx + 10 ) );
   test( "moveBy #4", win.screenY, ( oldy + 5 ) );
