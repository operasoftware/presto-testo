...
var cvs = "$Id: js_document.js 113915 2012-04-17 14:00:33Z svn $";
...
tprop( 'anchors', 'object' );
tprop( 'applets', 'object' );
...
//tprop( 'layers', 'object' );	// Netscape 4, nobody else supports it
tprop( 'links', 'object' );
tprop( 'open', 'function' );
...
if (!isMozilla())
{
   ...
   //tprop( 'all', 'undefined' ); // undetectable enough to be untestable here
...
test( 'captureEvents #1', document.captureEvents ? document.captureEvents.length : -1, 0 ); // changed by Hallvord, October 2010
// we don't really support arguments to captureEvents and will never implement it
// so let's just expect 0 and be done with it
...
if (get_protocol_and_host().indexOf("file://") == -1)
{
  ...
  var cookie_was_set_correctly=document.cookie.indexOf(mycookie)!=-1;
  test( 'cookie set? #1', cookie_was_set_correctly, true );
}
...
test_color( 'fgColor as assigned #1', document.body.text, 'white', 83721 );
document.fgColor=colornames.yellow;
test_color( 'fgColor as assigned #2', document.body.text, 'yellow', 83721 );
...
// one produced by MSIE is equivalent.
   ...
   test_spaceagnostic( 'onclick #2', "" + document.body.onclick,
                       "function () { void(3); }" );
