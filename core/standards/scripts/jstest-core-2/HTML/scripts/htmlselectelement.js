...
var cvs = "$Id: htmlselectelement.js 4838 2006-01-18 05:59:01Z hallvord $";
...
try {
  ...
  sel.selectedIndex = 0;
  test("selectedIndex",sel.selectedIndex,0);
  sel.selectedIndex = 2;
  test("value",sel.value,"der");
  sel.disabled = false;
  test("disabled",sel.disabled,false);
  ...
  test("multiple",sel.multiple,false);
  sel.multiple = true;
  test("multiple",sel.multiple,true);
  sel.size = 3;
  test("size",sel.size,3);
  sel.tabIndex = 0;
  test("tabIndex",sel.tabIndex,0);
  ...
  expect_DOM_exception( "type", DOMException.NO_MODIFICATION_ALLOWED_ERR, function(){sel.type = "select-multiple";});
